...
launchd_enable() {
    local service="$1"
    local launchd_disabled_plist="/Volumes/Data/private/var/db/com.apple.xpc.launchd/disabled.plist"
    test -f $launchd_disabled_plist || plutil -create binary1 $launchd_disabled_plist
    plutil -replace $service -bool false $launchd_disabled_plist
}
...
if [ "$major_version" -lt 27 ]; then
    ...
    launchd_enable "com\.openssh\.sshd"
...
launchd_enable "com\.apple\.screensharing"
